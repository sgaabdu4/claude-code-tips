...
- **`hooks/` `commands/` `rules/*.md` `bin/` `statusline/` → flat copy** (`cp_with_backup`, `install.sh:283`): per-file. If the target exists and differs, it's renamed to `<name>.bak.<ts>.<pid>` before overwrite; identical files are a no-op.
...
- **`agents/` is absent by design.** Subagent definitions are private and live in `~/.claude/agents/` on the user's machine. `bin/sync-runner-tools.mjs` and `sync-runner-tools-on-edit` target `~/.claude/agents/e2e-*-runner.md` *at runtime* — the file they rewrite is not in this repo. The `/e2e*` commands likewise load an `e2e-protocol` skill that lives outside this repo.
